...
### Decorators
A decorator is a special type of control node that only has one child and are designed to transform a node's return value. Examples of commonly-used decorators are Invert, which inverts the response of its child, and Fail or Succeed which always return a specific response.

## How to Use a Behavior Tree for AI role playing
Whenever you attempt to send a new chat message, this extension will capture the recent chat history and send it to a secondary AI, which will read the history and query it with YES/NO questions. The answers to these queries not only direct the traversal through branches of the Tree, but they can also modify variables or add scenarios modifiers to a block of text that will be injected into the context of the chat. At the end of the Tree traversal the compiled response will be added to the context as a system message and can help direct the scenario.

For example, here is a Sequence that would allow you to open a locked treasure chest only if you have found the Golden Key. In a Sequence, each node is run unless one of them returns FAILURE. If {{user}} has the key and is trying to open the chest, then the scenario modifier will be added to the response.
...
The first node checks to see if you have picked up the Golden Key. The cacheYes flag here will make it so that the first time this query response is YES, it will save this in the persistent state data and skip the query in the future. This state data is useful in preserving information that the AI might forget about if the conversation is long. The second node checks to see if we are trying to open the chest. If true, then the last node will add some text to the response that tells the AI that it is okay to unlock the chest.
...
```
This may seem like a lot of work for a single treasure chest, but the queries can be adapted to handle anything with a lock.
...
Currently KoboldCpp is supported, but other APIs can be added by request.

### View Tree
View and edit the currently loaded Behavior Tree. This editor is good for small modifications, but for authoring a Tree, I recommend an editor like VSCode or Notepad++. Furthermore, if your Tree gets corrupted somehow, there is no good way to restore it, so it is best to edit your Trees with a separate program. When you save the Tree, it will be saved for the currently-selected character card and will be reloaded every time you load this card.
...
### JSON File Format
The behavior tree files are stored in JSON format. This is a text file format that can be edited with the editor of your choice. I prefer VSCode since it does automatic syntax checking for you. There are also websites that will check your JSON for errors. I recommend using one of those sites if you aren't using VSCode or some other editor that checks JSON syntax.

### Response
The response from this extension is a block of text that will be injected into the chat context near the bottom. The location is configurable in the settings. The response contains some preamble text, then a list of vars with their values, then a list of scenario modifiers. It will be injected into the prompt as a system message. 
...
The response is cached between runs and if you resubmit the same chat history to the extension, such as for swipes, then the cached response will be returned instead of running the Tree. If you want to force the Tree to run again for a swipe, you can edit a previous message and make any trivial change.
...
> NOTE: If you set a var to 0 it will still appear in the response as '0%'. If you set it to -1, then it won't be in the response.
...
> NOTE: State gets cleared whenever the chat changes. This may be fixed in the future.

### Tasks

#### QueryTask
This the most important node that you will use to customize your scenarios. The responses from queries are what allows the Tree to branch and follow different paths and activate different scenario effects.
...
Name is optional, but recommended, especially if you are using the cacheYes or cacheNo flags since the name will be used as the state key. Data contains the query that will be sent to the summarization AI. The cacheYes and cacheNo flags are optional and if set, the first time the query returns a YES (for cacheYes) or NO (for cacheNo) the result will be persisted in the state data. Future runs will skip the query if there is a cached result.

The yes and no blocks are optional response actions which allow you to influence the response based on the result of the query. You can set one or more vars, and you can add one or more scenario modifiers to the response.

#### CreateVarsTask
This node defines one or more vars that will be used by this Tree. Each var has a keyword that defines it, a starting value which is set at the beginning of each Tree run, and a prompt which is added to the response.
...
```
Values are in percentages and can range from -1 to 100. If the value of a var is -1, then it will be excluded from the response. For any other number, the response will contain:
`{prompt} {value}%`
...
One practical use-case for these could be at the top of the Tree to set up some vars that should be applicable to the entire scenario. Maybe you have a query that says, "Has {{char}} turned into a werewolf?" and if so, you want to set a var "evilchar": 100, but you don't want this result to affect the rest of the Tree otherwise. In this case you could put it in a sequence with a "succeed" decorator which would allow the Tree to continue normally regardless of the answer to the query.

> NOTE: These decorators do not affect their child node, only the value that is returned to its parent.
